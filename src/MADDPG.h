...
public:
    ...
    MADDPG(Environment *sim, int64_t Ain_dims, int64_t Aout_dims, std::vector<int64_t> Ah_dims, int64_t Cin_dims,
           int64_t Cout_dims, std::vector<int64_t> Ch_dims, size_t n_agents, size_t scenario, float alpha=0.01, 
           float beta=0.01, size_t fc1=64, size_t fc2=64, size_t T=20, float gamma=0.99, float tau=0.01, 
           std::string path="", size_t batch_size=256,size_t max_memory=100000, size_t k_epchos=1000);
    void saveCheckpoint();
    void loadCheckpoint();
